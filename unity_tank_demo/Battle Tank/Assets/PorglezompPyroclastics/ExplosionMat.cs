...
public class ExplosionMat : MonoBehaviour {
	...
	void Start () {
        ...
        rendererObj.material = new Material(ExplosionMaterial);
	...
	void Update () {
		if (doUpdate) {
			Material rsm = rendererObj.sharedMaterial;
	...
	void SetShaderKeywords () {
		...
		rendererObj.sharedMaterial.shaderKeywords = newKeywords.ToArray();
	}
}
