...
namespace MATNet
{
    public class UPnP
    {
        ...
        public string name = "MATNetServer";
        ...
        public IPAddress gatewayAddress { get; private set; } = IPAddress.None;

        /// <summary>
        /// Whether there are threads active.
        /// </summary>

        public bool hasThreadsActive { get { return mDiscover != null || mThreads.Count > 0; } }
        ...
        public void WaitForThreads()
        {
            ...
            while (mDiscover != null || mThreads.Count > 0)
            {
        ...
        void ThreadDiscover(object obj)
        {
            ...
            List<IPAddress> ips = MNTools.localAddresses;
            ...
            for (int i = 0; i < ips.Count; ++i)
            {
                ...
                try
                {
                    ...
                    for (; ; )
                    {
                        ...
                        if (ParseResponse(Encoding.ASCII.GetString(data, 0, data.Length)))
                        {
                            ...
                            gatewayAddress = sourceAddress.Address;
                ...
                catch (Exception) { if (sender != null) sender.Close(); }
        ...
        {
            string response = MNTools.GetResponse(WebRequest.Create(url));
        ...
        string SendRequest(string action, string content, int timeout, int repeat)
        {
            ...
            try
            {
                for (int i = 0; i < repeat; ++i)
                {
                    ...
                    string response = MNTools.GetResponse(web);
        ...
        void Open(int port, bool tcp, OnPortRequest callback)
        {
            if (port > 0)
            {
                ...
                if (!mPorts.Contains(id) && status != Status.Failure)
                {
                    if (status == Status.Inactive)
                    {
                        Start();
                    ...
                    string addr = MNTools.localAddress.ToString();
        ...
        void Close(int port, bool tcp, OnPortRequest callback)
        {
            ...
            if (port > 0 && mPorts.Remove(id) && status == Status.Success)
            {
                ExtraParams xp = new ExtraParams();
                xp.callback = callback;
                xp.port = port;
            ...
            {
                callback?.Invoke(this, port, tcp ? ProtocolType.Tcp : ProtocolType.Udp, false);
            }
        }
        ...
        {
            while (status == Status.Searching) Thread.Sleep(1);
        ...
        {
            string response = (status == Status.Success) ? SendRequest(xp.action, xp.request, 10000, 3) : null;
            xp.callback?.Invoke(this, xp.port, xp.protocol, !string.IsNullOrEmpty(response));
